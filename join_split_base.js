...
const JOIN_SPLIT_CONFIG = {
  ...
  PRIMARY_AUDIO_TIELINE_INPUT_FROM_SEC_ID: 8, // change only for non-standard singe screen setups
...
General microphones and video sources
*/
...
let OVERVIEW_DOUBLE_SOURCE_IDS = [1,3];
...
// THESE SHOULD NOT BE CONFIGURED BY THE INSTALLER
  ...
  // TODO: Validate with Enrico the below settings for primary default config.
...
// TODO: Need to edit makeCameraSwitch() so that it only switches between video input 1 (local quadcam) and PRIMARY_VIDEO_TIELINE_INPUT_FROM_SEC_ID
// and in PresenterTrack QA mode it composes PresenterTrack camera source and input 1 or PRIMARY_VIDEO_TIELINE_INPUT_FROM_SEC_ID
...
function setComposedQAVideoSource(connectorDict) {
  ...
  if (connectorDict.ConnectorId[1]==QUAD_CAM_ID) {
...
async function updateUSBModeConfig() {
  ...
  // we no longer need to delegate the hiding of panel_combine_split to the USB Mode macro
  // since we are doing it natively here
  /* 
  object = { AlterUSBConfig: { config: 'hideCustomPanels_inUSBMode', value: true } }
  await localCallout.command(object).post() 
  object = { AlterUSBConfig: { config: 'hideCustomPanels_inUSBMode_PanelIds', value: ['panel_combine_split'] } }
  await localCallout.command(object).post()
  */
}
              ...
              case 'join_split_base':
                if (event.Type == 'Error') {
                  ...
                  switch (event.Value) {
                    ...
                    break;
                    case 'automatic_mode':
                      // handle request to keep speakertrackon from primary to secondary
...
// ID of HDMIN input a configurable constant below
 ...
 await storeSecondarySettings(ultraSoundMaxValue, standbyWakeupMotionValue, standbyControlValue);
  ...
  xapi.command('Video Matrix Assign', { Output: JOIN_SPLIT_CONFIG.PRIMARY_VIDEO_TIELINE_INPUT_FROM_SEC_ID, SourceID: 1 }).catch((error) => { console.error(error); });
